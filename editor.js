document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLanguage(lang) {
        ...
        document.querySelectorAll('[data-i18n]').forEach(el => {
            ...
            if (i18n[currentLang].hasOwnProperty(key)) {
                // HTMLタグを含むキーはinnerHTMLで更新
                if (['important_bug_report', 'load_instruction', 'important_conversion_feature_note'].includes(key)) {
    ...
    function updateItemDatalist() {
        ...
        itemDatalist.innerHTML = ''; // 中身をクリア
        if (!itemData) return;

        Object.keys(itemData).forEach(itemId => {
            const itemName = getItemName(itemId, currentLang);
            const option = document.createElement('option');
            option.value = itemId;
            option.textContent = `${itemId} : ${itemName}`;
